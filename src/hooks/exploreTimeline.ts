import type { ExploreResult } from "@/domain/explore";
...
export const EVENT_DURATIONS: Record<ExploreEventType, number> = {
  STAGE_ENTRY: 700,
  MONSTER_ENTRY: 560,
  ATTACK_SWING: 450,
  HIT_REACT: 500,
...
export const getEventDuration = (durationMs: number, speedMode: ExploreSpeedMode): number => {
  if (speedMode === "normal") {
    return durationMs;
  }
...
export const buildExploreTimelineEvents = ({
  ...
  if (!ended) {
    const endText = result.endReason === "DEFEATED" ? `${floor}-${result.clearedStage} 탐험 실패` : `${floor}층 탐험 완료`;
    events.push(
